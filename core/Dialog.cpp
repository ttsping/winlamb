...
INT_PTR CALLBACK Dialog::Proc(HWND hDlg, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	if (msg == WM_INITDIALOG) {
		pObj = (Dialog*)lp;
		pObj->hw = hDlg;
		SetWindowLongPtr(hDlg, DWLP_USER, (LONG_PTR)pObj);
		EnumChildWindows(hDlg, [](HWND hCtrl, LPARAM lp) -> BOOL {
			SendMessage(hCtrl, WM_SETFONT, (WPARAM)(HFONT)lp, MAKELPARAM(FALSE, 0));
			return TRUE;
		}, (LPARAM)core::Font::UiFont().hFont());
	} else {
		pObj = (Dialog*)GetWindowLongPtr(hDlg, DWLP_USER);
	}
