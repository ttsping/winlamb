...
public:
	...
	template<typename C,
		typename = std::enable_if_t<
			std::is_base_of_v<IUnknown, C>
		>>
	[[nodiscard]] ptr<C> query_interface(REFIID iid_any)
	{
		ptr<C> ptrBuf;

		if (HRESULT hr = this->_ptr->QueryInterface(iid_any,
				reinterpret_cast<void**>(ptrBuf.raw_pptr()));
			FAILED(hr))
		{
			throw std::system_error(hr, std::system_category(),
				"QueryInterface() failed in " __FUNCTION__ "().");
		}
		return ptrBuf;
	}
	...
	[[nodiscard]] ptr<C> query_interface()
	{
		...
		if (HRESULT hr = this->_ptr->QueryInterface(IID_PPV_ARGS(ptrBuf.raw_pptr()));
			FAILED(hr))
		{
			throw std::system_error(hr, std::system_category(),
				"QueryInterface() failed in " __FUNCTION__ "().");
	...
	REFCLSID clsid_any, REFIID iid_any, DWORD clsctxContext = CLSCTX_INPROC_SERVER)
{
	...
	if (HRESULT hr = CoCreateInstance(clsid_any, nullptr,
			clsctxContext, iid_any, reinterpret_cast<LPVOID*>(ptrBuf.raw_pptr()));
		FAILED(hr))
	{
		throw std::system_error(hr, std::system_category(),
			"CoCreateInstance() failed in " __FUNCTION__ "().");
	}
	return ptrBuf;
}
